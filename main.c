#include <pspkernel.h>
#include <pspctrl.h>
#include <pspdisplay.h>
#include <psprtc.h>

#define NBR_ROWS 24
#define NBR_PIECES 4
#define BLOCK_SIZE 10

void sceDmacMemcpy(void *dst, const void *src, int size);
int atexit(void(*function)(void)) {
    function();
    return 0;
}
void exit(int status) {
    while(1);
}

PSP_MODULE_INFO("tl", 0, 1, 1);
PSP_MAIN_THREAD_ATTR(THREAD_ATTR_USER);
PSP_HEAP_SIZE_KB(-1024);

static u16 __attribute__((aligned(16))) rows[NBR_ROWS] = {0};

static u64 __attribute__((aligned(16))) pieces[NBR_PIECES] = {
    0b0000000010000000000000001000000000000000100000000000000010000000,
    0b0000000010000000000000111000000000000000000000000000000000000000,
    0b0000000010000000000000011100000000000000000000000000000000000000,
    0b0000000011000000000000011000000000000000000000000000000000000000
};

static u32* const vram[3] = {
    ((u32*)0x44000000),
    ((u32*)0x44088000)
};

void drawBlock(const u8 x, const u8 y) {
    u8 i = BLOCK_SIZE;
    while(--i) {
        u8 j = BLOCK_SIZE;
        while(--j) {
            vram[0][(i+x*BLOCK_SIZE) | ((j+y*BLOCK_SIZE) << 9)] = 0xFFFFFFFF;
        }
    }
}

void drawRow(const u16 value, const u8 y) {
    u8 x = 16;
    while(x--) {
        if(value & (0x8000 >> x)) {
            drawBlock(x, y);
        }
    }
}

void draw4Rows(const u64 value, u8 y) {
    u8 x = 64;
    while(x--) {
        const u8 _x = (x % 16);
        if(!_x) {
            y++;
        }
        if(value & (0x8000000000000000 >> x)) {
            drawBlock(_x, y);
        }
    }
}

u8 isCollid(u16* const target, const u64 piece) {
    return (*(u64*)target | piece) > (*(u64*)target ^ piece);
}

int main() {
    SceCtrlData pad;
    sceDisplaySetMode(1, 480, 272);
    sceDisplaySetFrameBuf(vram[0], 512,
    PSP_DISPLAY_PIXEL_FORMAT_8888, PSP_DISPLAY_SETBUF_IMMEDIATE);

    u8 step = 0;
    u64 piece = 0, keytick = 0, runtick = 0;

    sceRtcGetCurrentTick(&runtick);
    keytick = runtick;

    const u64 tickres = sceRtcGetTickResolution() / 1000;    

    do {
        sceDmacMemcpy(vram[0], vram[1], 512*272*4);
        sceCtrlReadBufferPositive(&pad, 1);

        u64 tick = 0;
        sceRtcGetCurrentTick(&tick);

        if(!piece) {
            piece = pieces[tick  % 4];
            step = 0;
        }

        draw4Rows(piece, step);

        if((tick - runtick) / tickres >= 250) {
            step++;
            runtick = tick;
        }

        u16* const target = &rows[step];

        if((tick - keytick) / tickres >= 125) {
            if(pad.Buttons & PSP_CTRL_LEFT) {
                piece <<= 1;
            } else if(pad.Buttons & PSP_CTRL_RIGHT) {
                piece >>= 1;
            }
            keytick = tick;
        }

        if(step >= NBR_ROWS - 4 || isCollid(target, piece)) {
            *(u64*)(target-1) |= piece;
            piece = 0;
        }

        u8 i = NBR_ROWS;
        while(i--) {
            drawRow(rows[i], i);
        }

        sceDisplayWaitVblankStart();
    } while(!(pad.Buttons & PSP_CTRL_SELECT));
    sceKernelExitGame();
    return 0;
}
